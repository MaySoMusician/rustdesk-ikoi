...
def main():
    ...
    if windows:
        ...
        if osx:
            os.system(
                'strip target/release/bundle/osx/RustDeskIkoiStudents.app/Contents/MacOS/rustdesk-ikoi')
            os.system(
                'cp libsciter.dylib target/release/bundle/osx/RustDeskIkoiStudents.app/Contents/MacOS/')
            # https://github.com/sindresorhus/create-dmg
            os.system('/bin/rm -rf *.dmg')
            plist = "target/release/bundle/osx/RustDeskIkoiStudents.app/Contents/Info.plist"
            txt = open(plist).read()
...
#rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RustDeskIkoiStudents.app/Contents/MacOS/rustdesk
#rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RustDeskIkoiStudents.app/Contents/MacOS/libsciter.dylib
#rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RustDeskIkoiStudents.app
# goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/RustDeskIkoiStudents.app/Contents/MacOS/*
codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/RustDeskIkoiStudents.app
'''.format(pa))
            os.system('create-dmg target/release/bundle/osx/RustDeskIkoiStudents.app')
            os.rename('RustDeskIkoiStudents %s.dmg' % version, 'rustdesk-%s.dmg' % version)
            if pa:
                os.system('''
